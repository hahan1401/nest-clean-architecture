...
| Calendar dates | `@db.Date` — no time, no zone. Materialise as **UTC midnight** |
...
> **Date handling.** Build `@db.Date` values from date-only ISO strings
> (`new Date('2027-02-14')`) or `Date.UTC(...)`, and read weekdays with `getUTCDay()` so they
> match Postgres `EXTRACT(DOW)`. `new Date(2027, 1, 14)` is local time and shifts the night
> by one outside UTC.
...
| `departure_date` | `date` | UTC midnight |
...
| `tour_id` | `int?` FK → `tours.id` | cascade |
| `start_date` / `end_date` | `date?` | inclusive window; NULL on a side = unbounded there |
...
| `check_in` / `check_out` | `date?` | half-open `[check_in, check_out)`; `check_out > check_in` |
...
- `bookings_room_no_overlap` — **the** invariant:
  ...
  room, regardless of application correctness. The half-open range makes same-day turnover
  legal (a checkout and a check-in on the 14th do not collide), and the partial predicate
...
| `line_date` | `date` | a stay night (`check_in` … `check_out - 1`) for rooms; the departure date for tours |
