...
import { USER_SERVICE, USER_PATTERNS, CreateUserDto, UpdateUserDto, UpdateLocationDto } from '@app/common';
...
export class UserController {
  ...
  async create(@Body() dto: CreateUserDto) {
    return lastValueFrom(this.userClient.send(USER_PATTERNS.CREATE_USER, dto))
      .catch((err) => { throw new HttpException(err?.message ?? 'Internal error', err?.status ?? 500); });
  }
  ...
  async update(@Req() req: Request, @Param('id') id: string, @Body() updateData: UpdateUserDto) {
    return lastValueFrom(this.userClient.send(USER_PATTERNS.UPDATE_USER, { id, updateData, requestId: req['requestId'] }))
      .catch((err) => { throw new HttpException(err?.message ?? 'Internal error', err?.status ?? 500); });
  }
  ...
  async updateLocation(@Param('id') id: string, @Body() dto: UpdateLocationDto) {
    return lastValueFrom(
      this.userClient.send(USER_PATTERNS.UPDATE_LOCATION, { id, latitude: dto.latitude, longitude: dto.longitude }),
    ).catch((err) => { throw new HttpException(err?.message ?? 'Internal error', err?.status ?? 500); });
  }
