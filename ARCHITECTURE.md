# Clean Architecture - NestJS Microservices + TypeORM
...
│   ├── database/      # DatabaseModule (TypeORM) + ORM entities + domain entities
...
│      (TypeORM repos, external adapters: HTTP/VNPay)      │
└──────────────────────────────────────────────────────────┘
```
...
- Uses TypeORM (`@app/database`) and external integrations (Nominatim, VNPay, Gemini).
...
- `DatabaseModule` (configures the TypeORM Postgres connection from `DATABASE_URL`)
- `UserOrmEntity` (TypeORM persistence model) and the `User` / `UserWithDistance` domain models
- Repositories inject the TypeORM `DataSource` for typed access and raw SQL (Haversine, pgvector)
...
This keeps application logic independent of TypeORM and transport details.
